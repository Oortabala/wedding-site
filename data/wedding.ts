export const wedding = {
  couple: {
    groom: "Олжас",
    bride: "Айтолқын",
    title: "Олжас & Айтолқын",
  },

  event: {
    type: "Үйлену той",
    date: "2026-06-13T18:00:00",
    displayDate: "13 маусым 2026",
    day: "13",
    month: "маусым",
    weekday: "Сенбі",
    time: "18:00",
    city: "Шымкент",
  },

  invitation: {
    greeting: "Құрметті ағайын-туыс, құда-жекжат, дос-жаран!",
    text: "Сіздерді балаларымыз Олжас пен Айтолқын келініміздің үйлену тойына арналған ақ дастарханымыздың қадірлі қонағы болуға шақырамыз.",
    from: "Ізгі ниетпен: Даулет & Перизат",
  },

  venue: {
    name: "Shymkent Hall",
    address: "Шымкент қ., Проспект Байдибек би, 246/3",
    gisLink: "https://go.2gis.com/BSofa",
    googleMapsLink: "https://maps.app.goo.gl/zKW7xbAaPA2VETiB9",
  },

  timeline: [
    { time: "17:30", title: "Қонақтарды қарсы алу" },
    { time: "18:00", title: "Беташар" },
    { time: "19:00", title: "Тойдың басталуы" },
    { time: "22:00", title: "Торт" },
    { time: "23:30", title: "Финал" },
  ],

  images: {
    couple: "/images/olzhas-aitolkyn/couple-1.webp",
    petals: "/images/olzhas-aitolkyn/petals.webp",
    birds: "/images/olzhas-aitolkyn/birds.webp",
  },

  rsvp: {
  formUrl: "https://tally.so/r/7R1640",
}
};